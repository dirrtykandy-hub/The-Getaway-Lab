...
# BUN_INSTALL workaround: the user running this may not own the default bun
# install cache dir (/home/engine/.bun/install is root-owned), so redirect it
# to a writable temp location.
